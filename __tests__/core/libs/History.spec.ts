...
describe('History', () => {
  ...
  test('should add history records to a user', () => {
    const ssid = 'user1'
    const record1: HistoryRecord = { role: 'user', type: 'text', user: ssid, content: 'Hello' }
    const record2: HistoryRecord = { role: 'assistant', type: 'text', user: ssid, content: 'Hi' }
  ...
  test('should limit the number of history records per user', () => {
    ...
    const record1: HistoryRecord = { role: 'user', type: 'text', user: ssid, content: 'Hello' }
    const record2: HistoryRecord = { role: 'assistant', type: 'text', user: ssid, content: 'Hi' }
    const record3: HistoryRecord = { role: 'user', type: 'text', user: ssid, content: 'How are you?' }
    const record4: HistoryRecord = { role: 'assistant', type: 'text', user: ssid, content: 'I am fine' }
